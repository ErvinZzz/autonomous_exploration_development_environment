...
class Follower():
    def __init__(self):
        ...
        self.cmd_vel_pub = rospy.Publisher('/X1/cmd_vel', Twist, queue_size=5)

        self.position_subscriber = rospy.Subscriber('/cmd_vel', TwistStamped, self.set_cmd_vel, queue_size=5)

        rospy.loginfo("Subscribing to twiststamped......")

        # Wait for the  topic to become available
        rospy.wait_for_message('/cmd_vel', TwistStamped)
