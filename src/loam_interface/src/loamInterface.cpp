...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  if (sendTF) {
    if (!reverseTF) {
      ...
      tfBroadcasterPointer->sendTransform(tf::StampedTransform(odomTrans.inverse(), odom->header.stamp, "/X1", "/map"));
    }
  }
}
